...
# Section-specific toggle (May or June only)
def section_toggle(section_title: str):
    return st.radio(f"{section_title} – Select Month", ["May", "June"], horizontal=True, key=f"toggle_{section_title}")

# Chart display function using section month
def show_html_chart(title: str, may_file: str, june_file: str = None, height: int = 600, month: str = "May"):
    st.subheader(title)
...
col1, col2 = st.columns(2)
with col1:
    show_html_chart("Party Affiliation", "may_party.html", "june_party.html", month=demographics_month)
with col2:
    show_html_chart("Age Distribution", "may_age_group.html", "june_age_group.html", month=demographics_month)

with st.expander("More Demographics", expanded=False):
    more_demo_month = section_toggle("More Demographics")
    col3, col4 = st.columns(2)
    with col3:
        show_html_chart("Gender", "may_gender.html", "june_gender.html", month=more_demo_month)
        show_html_chart("Family Income", "may_family_income.html", "june_family_income.html", month=more_demo_month)
    with col4:
